// Adapted from Erdet Nasufi's to work with new vlang 0.4.7
...
pub fn new_port(name string) !Port {
	mut this := Port{
		ptr: voidptr(0)
	}
	this.name = name
	cfn := &char(name.str)
	unsafe {
		rc := C.sp_get_port_by_name(cfn, &(this.ptr))
		if rc != Return(C.SP_OK) {
			return error('Failed to create new Port by name "${name}". Error: ${rc}')
		}
	}
	return this
}
...
pub fn (this Port)name() string {
	unsafe {
		return cstring_to_vstring(C.sp_get_port_name(this.ptr))
	}
}

pub fn (this Port)description() string {
	unsafe {
		return cstring_to_vstring(C.sp_get_port_description(this.ptr))
	}
}

pub fn (this Port)transport() Transport {
	return Transport(C.sp_get_port_transport(this.ptr))
}
...
pub fn (this Port)read(mode ReaderMode, max_length u32, timeout_ms u32) []u8 {
	mut rc := int(0)
	if max_length == 0 {
		return []u8{}
	}

	mut buff := []u8{len: int(max_length), cap: int(max_length)}
	match mode {
		.blocking {
			rc = unsafe {
...
pub fn (this Port)write(block bool, buffer []u8, timeout_ms u32) !int {
	mut rc := int(0)
...
pub fn (this Port)signal() !Signal{
	unsafe {
		csig := Signal(0)
		rc := C.sp_get_signals(this.ptr, &csig)
...
pub fn version_pkg_major_minor_micro() (int, int, int) {
	return C.sp_get_major_package_version(),
		C.sp_get_minor_package_version(),
		C.sp_get_micro_package_version()
}

pub fn version_lib_current_revision_age() (int, int, int) {
	return C.sp_get_current_lib_version(),
		C.sp_get_revision_lib_version(),
		C.sp_get_age_lib_version()
}
